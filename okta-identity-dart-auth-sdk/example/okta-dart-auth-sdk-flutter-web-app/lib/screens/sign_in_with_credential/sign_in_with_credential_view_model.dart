...
class SignInWithCredentialViewModel extends ChangeNotifier {
  ...
  Future<void> signInWithCredential(VoidCallback onSuccess) async {
    try {
      ...
      await okta -
          identityApp.okta -
          identityAuth?.signInWithCredential(credential);
  ...
  Future<void> loginWithFacebook(BuildContext context) async {
    ...
    if (result.status == LoginStatus.success) {
      ...
      try {
        var user = await okta -
            identityApp.okta -
            identityAuth?.linkAccountWithCredientials(
                'http://localhost', accessToken.token, 'facebook.com');

        var user1 = await okta -
            identityApp.okta -
            identityAuth?.signInWithRedirect(
                'http://localhost', accessToken.token, 'facebook.com');
      ...
      // Use this token to authenticate with your backend or okta-identity
